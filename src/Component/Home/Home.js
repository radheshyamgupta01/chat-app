...
function Home() {
  ...
  const { userHandler, user_idData } = useContext(Context);
  const [senderMessageData, setsenderMessageData] = useState([]);
  const [recepientMessageData, setRecepienMessagetData] = useState([]);
  ...
  const handlePostMessage = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:4000/user/createChat", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          message: chatMessage,
          recipientUserId: RecipientUser[0],
        }),
        // body: JSON.stringify({
        //   content:chatMessage,
        //   recipient_id: RecipientUser[0],
        // }),
      });
      ...
      if (response.ok) {
        ...
        //         console.log(data.createdAtBysender,"this is created by sender")
        const data = await response.json();

        console.log(data.createdAtBysender, "this is created by sender");
        setsenderMessageData(data.createdAtBysender);
        setRecepienMessagetData(data.createdAtByReceipient);
        console.log(senderMessageData, "this is sender messafe");
        ...
        console.error("Failed to post chat messagekkk.");
      }
    } catch (error) {
      console.error("An error occurred while posting chat message:", error);
    }
  };

  const getAllChat = async (user,name) => {
    
    try {
      const response = await fetch(
        `http://localhost:4000/user/getAllChat?recipientUserId=${RecipientUser[0]}&recipientUserName=${RecipientUser[1]}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${userId}`,
      ...
      if (response.ok) {
        ...
        console.log(
          data,
          "this is data which I'm getting when I refresh the page "
        );
                ...
                <div class="flex flex-row items-center p-4 bg-gradient-to-r from-red-100 to-transparent border-l-2 border-red-500">
                  ...
                  <div class="flex flex-col flex-grow ml-3">
                    <div class="flex items-center">
                      ...
                      <div class="h-2 w-2 rounded-full bg-blue-500 ml-2"></div>
                    </div>
                    <div class="text-xs truncate w-40">
                      Lorem ipsum dolor sit amet, consectetur adipisicing elit.
                      Debitis, doloribus?
        ...
        <div class="flex flex-col h-full w-full bg-white px-4 py-6">
          <div class="flex flex-row items-center py-4 px-6 rounded-2xl shadow">
            <div class="flex items-center justify-center h-10 w-10 rounded-full bg-pink-500 text-pink-100">
              {RecipientUser[1] && RecipientUser[1].charAt(0).toUpperCase()}
            </div>
            <div class="flex flex-col ml-3">
              <div class="font-semibold text-sm">{RecipientUser[1]}</div>
                    ...
                    <div class="space-y-4">
                      ...
                      {!(data.senderEmail === currentUserEmail) && (
                        <div class="flex items-start mt-4">
                          <div class=" bg-gray-100 ml-3 p-3 rounded-lg text-black">
                            <p className="text-xs text-gray-500">
                              {data.senderMessage} <span className="underline">received by </span>{data.currentUserName}
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      {data.senderEmail === currentUserEmail ? (
                        <div class="flex items-end justify-end mt-4">
                          <div class=" bg-gray-100 p-3 rounded-lg">
                            <p className="text-xs  text-gray-500">
                              {" "}
                              {data.senderMessage}  <span className="underline">Sent by</span> {data.currentUserName}{" "}
                            </p>
                          </div>
                        </div>
                      ) : (
